...
local prepullImage = images.images.pihole.prepullImage;
local piholeImage = images.images.pihole.image;
...
{
  newPiholeEnvironment(namespace, cluster):: {
    local appConfig = cluster.defaultAppConfig + piholeAppConfig,
    ...
    dnsmasqVolume: {
      pvc: utils.newStandardPVC(dnsmasqVolName, '10Mi', appConfig.configVolStorageClass, labels),
    ...
    piholeDnsService: utils.newDnsService(appName + '-dns', selectorLabels, cluster.ipv4.pihole),
